...
- [x] **[low]** `init-admin` runs `schema.sql` via naive `;`-split, no transaction — `server/routes/setup.js:101` (the `auth.js generateToken` missing-`JWT_SECRET` half was already fixed).
- [x] **[low]** Schema-vs-code drift: `migrate.js`'s "Tables created" summary was a hardcoded list naming a dead `events` table — now reported from the live database — `server/config/migrate.js:28`.
