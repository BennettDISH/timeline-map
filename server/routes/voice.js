...
const { enabled, listVoices, speak, soundscape } = require('../voice/elevenlabs');
...
router.post('/nodes/:id/voice', wrap(async (req, res) => {
  ...
  await pool.query('UPDATE nodes SET voice_id=$1, voice_name=$2, updated_at=CURRENT_TIMESTAMP WHERE id=$3', [voiceId, voiceName, n.id]);
  res.json({ ok: true });
}));
...
router.post('/nodes/:id/line', wrap(async (req, res) => {
  const n = (await pool.query('SELECT id, world_id, title, voice_id FROM nodes WHERE id=$1', [req.params.id])).rows[0];
  if (!n || !(await ownsWorld(n.world_id, req.user.id))) return res.status(404).json({ message: 'Node not found' });
  ...
  const mp3 = await speak(n.voice_id, text);
  const key = `worlds/${n.world_id}/voice-${n.id}-${Date.now()}-${crypto.randomBytes(3).toString('hex')}.mp3`;
...
router.post('/maps/:id/ambience', wrap(async (req, res) => {
  ...
  const mp3 = await soundscape(prompt, Number(req.body?.seconds) || 20);
  const key = `worlds/${m.world_id}/ambience-${m.id}-${Date.now()}-${crypto.randomBytes(3).toString('hex')}.mp3`;
  const url = await putObject(key, mp3, 'audio/mpeg');
