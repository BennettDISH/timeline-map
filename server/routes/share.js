...
router.get('/:token/maps/:mapId', wrap(async (req, res) => {
  ...
  if (windowed) {
    ...
    rows = (await pool.query(
      `SELECT p.id AS placement_id, p.x, p.y, p.start_time, p.end_time, p.shape, p.shape_kind,
    ...
    rows = (await pool.query(
      `SELECT p.id AS placement_id, p.x, p.y, p.shape, p.shape_kind,
              n.id AS node_id, n.title, n.category, n.interior_map_id, n.pin, n.pin_size, n.author, n.visibility AS nvis,
              i.file_path AS node_image_path
       FROM placements p
       JOIN nodes n ON p.node_id = n.id
  ...
  const placements = rows.map((r) => ({
    id: r.placement_id, x: Number(r.x), y: Number(r.y), shape: r.shape || null, shapeKind: r.shape_kind || 'area',
